...
class ComponentToPrint extends React.Component {
  render() {
    const { semester, currentSchoolYear, instructor, subject, section, major, students } = this.props;
              ...
              <Box
                ...
                >
                  ...
                  <Box sx={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
                      ...
                      <Typography variant="body1" color="initial"><strong>CARLOS HILADO MEMORIAL STATE UNIVERSITY</strong></Typography>
                      <Typography variant="body1" color="initial">Main Campus, Talisay City, Negros Occidental</Typography>
                      <Typography variant="body1" color="initial"><strong>OFFICE OF THE REGISTRAR</strong></Typography>
                      <Typography variant="body1" color="initial">Student Grade Sheet</Typography>
              ...
              >
                <tbody className="tableHeaderOne">
                  <tr style={{ borderStyle: "hidden" }}>
                    <td colSpan={6}>Subject:: <strong>{subject}</strong></td>
                  </tr>
                  <tr style={{ borderLeftStyle: "hidden", borderRightStyle: "hidden" }}>
                    <td style={{ borderLeftStyle: "hidden", borderRightStyle: "hidden" }} colSpan={3}>Instructor :: <strong>{instructor}</strong></td>
                    <td style={{ borderLeftStyle: "hidden", borderRightStyle: "hidden" }} colSpan={3}>Curr / Yr / Sec :: <strong>{section}</strong></td>
                  </tr>
                </tbody>
                <tbody className="tableHeaderTwo">
                    ...
                    <tr>
                      <th></th>
                      <th align="left" colSpan={5}>{major || "---"}</th>
                    </tr>
                </tbody>
                <tbody border={1}>
                  {students.map(({studentName, midTermGrade, endTermGrade, finalGrade, remarks}, index) => (
                    <tr key={++index}>
               ...
               >
                <tbody>
                  <tr height={100}>
                    ...
                    <td></td>
                    <td></td>
                    <td style={{ textAlign:'center', verticalAlign:'bottom', fontSize: '14px' }} >{currentHeadOfRegistrar}</td>
                  </tr>
                  <tr  cellSpacing={10} style={{ borderBottom: '1px dotted black'  }}>
                    <td></td>
                    <td align='center' style={{ borderTop: '1px solid black' }} width={300}>Instructor's Signature</td>
                    <td align='center' style={{ borderTop: '1px solid black' }} width={300}>Dean</td>
                    <td align='center' style={{ borderTop: '1px solid black' }} width={300}>Registrar III</td>
                  </tr>
                  <tr>
                  </tr>
                  <tr className="rotal">
                    <td style={{ fontSize: '9px', padding: '.5rem', border: '1px solid black' }} width={100}>
  ...
  const PrintGradeSheet = () => {
    ...
    const ComponentToPrintProps = {
      semester: decode.semester === "summer" 
                ? "SUMMER" 
                : decode.semester === "1st" 
                ? "First Semester"
                : "Second Semester",
      currentSchoolYear: `${decode.currentSchoolYear} - ${parseInt(decode.currentSchoolYear) + 1}`,
